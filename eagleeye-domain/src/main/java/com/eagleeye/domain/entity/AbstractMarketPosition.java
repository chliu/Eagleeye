...
/**
 * Shared mapping for TAIFEX position tables (futures and options), which carry
 * an identical set of trading and open-interest volume/value columns.
 *
 * <p>Declared as a {@link MappedSuperclass} so each concrete entity keeps its
 * own table while inheriting these columns unchanged.
 */
@MappedSuperclass
public abstract class AbstractMarketPosition {
    ...
    protected AbstractMarketPosition() {}

    protected AbstractMarketPosition(LocalDate tradeDate, String contract, TraderType traderType) {
